...
def get_usable_openai_models():
    models_list = OpenAiClient.models.list()
    accesible_model_names = []
    for m in models_list:
        accesible_model_names.append(m.id)
    # NOTE, BUG: hacky workaround to find chat-conversation models
    # ...because openai provide no way of querying what endpoints
    # models can use (you have to try and when it fails you know)
    accessible_chat_models = [
        m for m in accesible_model_names if (("gpt" in m) and ("instruct" not in m))
    ]
    return accessible_chat_models
...
def find_match(input):
    ...
    return (
        (result["matches"][0]["metadata"]["source"].split("-")[1]).split(".")[0]
        + ": "
        + result["matches"][0]["metadata"]["text"]
        + "\n"
        + (result["matches"][1]["metadata"]["source"].split("-")[1]).split(".")[0]
        + ": "
        + result["matches"][1]["metadata"]["text"]
        + "\n"
        + (result["matches"][2]["metadata"]["source"].split("-")[1]).split(".")[0]
        + ": "
...
if __name__ == "__main__":
    print(OpenAiClient.models.list())
